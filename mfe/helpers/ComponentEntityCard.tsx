...
const ComponentEntityCard = ({
  ...
  const { mutate: deleteComponent } = useDeleteComponent({
    onSuccess: () => {
      ...
      showSnackbar(
        `Successfully removed anomaly detection component from asset type ${entity.id}`,
        "success"
      );
    },
    onError: (error) => {
  ...
  const getSelectedAttributes = () => {
    return JSON.parse(assetTypeData[0].schema)
      ...
      .map((attribute) => ({
        ...attribute,
        selected: (componentSettings[0].feature_attributes as string[]).find(
          (f) => f === attribute.attribute_name
        )
          ? true
          : false,
      }));
    ...
    // .filter(
    //   (attribute) =>
    //     !componentSettings[0].feature_attributes.find(
