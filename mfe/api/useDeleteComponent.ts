...
const deleteComponentFn = async (componentId: string, assetTypeId: string) => {
...
export function useDeleteComponent({ onSuccess, onError }: { onSuccess: () => void; onError?: (error: Error) => void }) {
  return useMutation(({ componentId, assetTypeId }: { componentId: string, assetTypeId: string }) => deleteComponentFn(componentId, assetTypeId), {
    onSuccess,
    onError,
  });
}
