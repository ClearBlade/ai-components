...
const createComponentFn = async (component: Record<string, any>, assetTypeId: string, settings: Record<string, any>) => {
    ...
    body: JSON.stringify({
      ...
      body: {
        item: {
          id: component.id,
          name: component.name,
          type: 'AI', //component.type,
          url: component.repo_url,
          prefix: component.prefix || '',
          entity_id: assetTypeId,
          enabled: true,
          settings: settings,
        }
      }
...
export function useCreateComponent({ onSuccess, onError }: { onSuccess: () => void; onError?: (error: Error) => void }) {
  return useMutation(({ component, assetTypeId, settings }: { component: Record<string, any>, assetTypeId: string, settings: Record<string, any> }) => createComponentFn(component, assetTypeId, settings), {
    onSuccess,
    onError,
  });
}
